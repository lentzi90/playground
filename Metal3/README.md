...
Before running anything in this directory, you need a Kind cluster named
`kubevirtbmc-test` (or set `CLUSTER_NAME` to match) with `/dev/kvm`
passthrough, plus [KubeVirt](https://kubevirt.io/) (with the
`DeclarativeHotplugVolumes` feature gate enabled),
[CDI](https://github.com/kubevirt/containerized-data-importer) (Containerized
Data Importer), and [cert-manager](https://cert-manager.io/) (used to issue
Ironic's TLS certificate) installed on it:

```bash
kind create cluster --name kubevirtbmc-test --config kind.yaml
...
# Install CDI. dev-setup.sh takes care of the StorageProfile for local-path
# storage (see Key design decision 5 above), so nothing more is needed here.

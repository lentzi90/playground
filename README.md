...
# If you want to autoscale from zero
export EXP_CAPO_AUTOSCALE_FROM_ZERO=true
...
source CAPO/.env
# Apply the clusterclass
clusterctl generate yaml --from https://github.com/kubernetes-sigs/cluster-api-provider-openstack/releases/download/v0.15.0-alpha.0/clusterclass-dev-test.yaml | kubectl apply -f -
...
clusterctl generate yaml --from https://github.com/kubernetes-sigs/cluster-api-provider-openstack/releases/download/v0.15.0-alpha.0/image-template-node.yaml | sed s/qcow2/raw/ | kubectl apply -f -

# Create a cluster
clusterctl generate cluster lennart-test --kubernetes-version=v1.35.6 --flavor=topology | kubectl apply -f -
# Label it for the ClusterResourceSets
kubectl label cluster lennart-test cni=calico cloud=openstack
```
...
providing the Redfish BMC in front of each VM.

Create the bootstrap/management Kind cluster with the prerequisites that
`Metal3/dev-setup.sh` expects already in place (KubeVirt, CDI, cert-manager):

```bash
kind create cluster --config Metal3/kind.yaml

# Install KubeVirt and enable the DeclarativeHotplugVolumes feature gate
export KUBEVIRT_VERSION="${KUBEVIRT_VERSION:-v1.9.0}"
...
# Install CDI (dev-setup.sh patches the StorageProfile for local-path storage)
export CDI_VERSION="${CDI_VERSION:-v1.66.1}"
kubectl apply -f "https://github.com/kubevirt/containerized-data-importer/releases/download/${CDI_VERSION}/cdi-operator.yaml"
kubectl apply -f "https://github.com/kubevirt/containerized-data-importer/releases/download/${CDI_VERSION}/cdi-cr.yaml"

# Install cert-manager (used to issue Ironic's TLS certificate)
